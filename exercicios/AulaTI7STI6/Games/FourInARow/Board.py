...
class Board(AbstractBoard):
	def __init__(self):
		super().__init__([[" "] * 8 for _ in range(8)])

	@staticmethod
	def __fourInARowCheck(slice: list[str]) -> tuple[True, str] | tuple[False, None]:
		oldPlayer = ""
		count = 0
	...
	def play(self, player: str = "", column: int = 1) -> tuple[int, int]:
		...
		self.state[lastEmptyRow][zeroIndexColumn] = player

		return lastEmptyRow + 1, column

	def inWinState(self, propagationPoint: tuple[int, int] = (1, 1)) -> tuple[True, str] | tuple[False, None]:
		...
		xyAxes = ("row", "col")
		for axis in xyAxes:
			result, player = self.__xyAxisCheck(axis, zeroIndexRowCoord, zeroIndexColCoord)
			if result: return result, player

		diagAxes = ("asc", "desc")
		for axis in diagAxes:
			result, player = self.__diagonalCheck(axis, zeroIndexRowCoord, zeroIndexColCoord)
			if result: return result, player
